...
项目目前仍处于 alpha 阶段，正在快速演进。这个仓库是编辑器、后端服务、CLI、VS Code 扩展、共享包、架构决策和实施规格的源码工作区。

## 项目目标

MdrFrontEngine 围绕几个长期约束构建：

- **MIR 作为唯一真相源**：UI、逻辑、动画、路由和代码生成都应收敛到经过校验的中间表示，而不是分散成各编辑器自己的状态。
- **可视化编辑不牺牲上限**：可视化工作流应支持真实代码、外部依赖、诊断和生产级代码生成，而不是停留在静态原型。
- **本地优先的工程体验**：编辑器应适合本地开发，同时支持后端 Workspace、同步和未来协作能力。
- **显式架构记录**：长期稳定的契约先记录在 `specs/` 中，再进入难以回退的实现细节。
