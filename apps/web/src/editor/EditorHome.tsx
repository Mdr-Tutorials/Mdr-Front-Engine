    ...
    <div className="relative flex h-full min-h-[280px] w-full flex-col rounded-[16px] border border-[var(--color-2)] bg-[var(--color-1)] p-[24px] text-left transition-all duration-[300ms] ease-[ease] hover:-translate-y-1 hover:border-[var(--color-4)] hover:bg-[var(--color-0)] hover:shadow-[var(--shadow-lg)]">
      ...
      >
        <div className="flex flex-col gap-[12px]">
          <div className="mb-[8px] text-[var(--color-primary)]">{getIcon()}</div>
...
function EditorHome() {
  ...
  const publishProject = async (project: ProjectSummary) => {
    const isClonedProject = /\(copy\)\s*$/i.test(project.name || '');
    if (!token || project.isPublic || busyByProject[project.id] || isClonedProject)
      return;
    setBusyByProject((prev) => ({ ...prev, [project.id]: 'publishing' }));
    try {
      const { project: published } = await editorApi.publishProject(token, project.id);
      setProjects((prev) =>
        prev.map((item) =>
          item.id === project.id
            ? {
