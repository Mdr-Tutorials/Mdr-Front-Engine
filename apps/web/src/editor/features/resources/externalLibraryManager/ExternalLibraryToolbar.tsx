    ...
    <>
      <div className="flex flex-wrap items-center gap-2">
        <input
          ...
          className="h-9 min-w-[220px] flex-1 rounded-lg border border-(--border-default) bg-(--bg-canvas) px-3 text-sm text-(--text-primary) outline-none focus:border-(--accent-color)"
          value={searchInput}
          onChange={(event) => onSearchInputChange(event.target.value)}
          placeholder={t('resourceManager.external.searchPlaceholder')}
        />
        <div className="inline-flex rounded-lg border border-(--border-default) bg-(--bg-canvas) p-1">
      ...
      <div ref={categoriesRef} className="flex flex-wrap items-center gap-1">
        ...
        <span
          aria-hidden="true"
          className="mr-2 ml-4 h-6 w-px bg-(--border-default)"
        />
        {builtinLibraryCategories.map((category) => (
          <div
            key={category.id}
            className="relative inline-flex items-center gap-1 rounded-lg bg-(--bg-canvas) px-2 py-1.5 text-xs text-(--text-secondary)"
          >
            ...
            <button
              ...
              className="inline-flex h-5 w-5 items-center justify-center rounded-full border-0 bg-transparent text-(--text-secondary) transition-colors hover:bg-(--bg-raised) hover:text-(--text-primary)"
              onClick={() => {
            ...
            {openCategoryId === category.id ? (
              <div
                ...
                className="absolute top-full left-0 z-30 mt-1 min-w-[180px] rounded-lg border border-(--border-subtle) bg-(--bg-canvas) p-1.5 shadow-(--shadow-lg)"
              >
                <div className="grid gap-0.5">
                  {category.libraryIds.map((libraryId) => (
                    <button
                      ...
                      className="rounded-md border-0 bg-transparent px-2 py-1 text-left text-xs text-(--text-secondary) transition-colors hover:bg-(--bg-raised) hover:text-(--text-primary)"
                      onClick={() => {
