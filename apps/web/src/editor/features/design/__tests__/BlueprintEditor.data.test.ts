...
import type { ComponentGroup, ComponentPreviewItem, ComponentPreviewOption, ComponentPreviewStatus } from "../BlueprintEditor.types"
...
describe("BlueprintEditor.data helpers", () => {
  it("selects the default size option", () => {
    ...
    expect(getDefaultSizeId([{ id: "XS", label: "XS", value: "Tiny" }])).toBe("XS")
  })
  ...
  it("detects wide components by group or item", () => {
    const group: ComponentGroup = { id: "layout", title: "Layout", items: [] }
    const item: ComponentPreviewItem = { id: "custom", name: "Custom", preview: null }
    expect(isWideComponent(group, item)).toBe(true)

    const narrowGroup: ComponentGroup = { id: "base", title: "Base", items: [] }
    const wideItem: ComponentPreviewItem = { id: "date-range-picker", name: "Range", preview: null }
