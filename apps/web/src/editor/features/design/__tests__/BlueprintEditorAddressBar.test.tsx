...
describe("BlueprintEditorAddressBar", () => {
  it("propagates input changes and adds routes", () => {
    ...
    fireEvent.change(screen.getByTestId("AddressInput AddressCurrentInput"), { target: { value: "/next" } })
    fireEvent.change(screen.getByTestId("AddressInput AddressNewInput"), { target: { value: "/draft" } })
  ...
  it("renders the route list", () => {
    ...
    expect(options.map((option) => option.textContent)).toEqual(["/", "/about"])
  })
})
