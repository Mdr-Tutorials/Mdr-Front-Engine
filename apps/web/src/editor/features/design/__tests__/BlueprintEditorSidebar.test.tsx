      ...
      items: [
        {
          ...
          variants: [
            { id: "primary", label: "Primary", element: <div>Primary</div> },
          ],
          renderPreview: () => <div data-testid="preview-render">Preview</div>,
        },
      ],
    },
  ],
...
describe("BlueprintEditorSidebar", () => {
  ...
  it("fires callbacks for group toggles and preview interactions", () => {
    ...
    const previewCard = screen.getByTestId("preview-render").closest(".ComponentPreviewCard")
    ...
    const statusDots = screen.getAllByRole("button", { name: /Default|Success/ })
    fireEvent.click(statusDots[1])
    expect(onStatusSelect).toHaveBeenCalledWith("button", 1)

    fireEvent.click(screen.getByRole("button", { name: "sidebar.expandVariants" }))
