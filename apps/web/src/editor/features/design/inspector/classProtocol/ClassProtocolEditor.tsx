...
export function ClassProtocolEditor({
    ...
    const resolveOverrideTarget = (index: number) => {
        ...
        for (let pointer = index + 1; pointer < tokens.length; pointer += 1) {
            ...
            if (
                mergedPair.includes(candidate) &&
                !mergedPair.includes(source)
            ) {
                return candidate;
            }
        }
                ...
                {tokens.map((token, index) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            {mountedCssTarget ? (
                                <span
                                    className="inline-flex h-1.5 w-1.5 shrink-0 rounded-full bg-black/55"
                                    aria-hidden="true"
                                    title={`Mounted CSS: ${mountedCssTarget.path}`}
                                    data-testid={`inspector-classname-mounted-dot-${index}`}
                                />
                            ) : null}
                ...
                <div className="relative">
                    <input
                        ...
                        onChange={(event) =>
                            onChange(
                                toClassNameValue(
                                    parseClassTokens(event.target.value)
                                )
                            )
                        }
                        placeholder={placeholder}
