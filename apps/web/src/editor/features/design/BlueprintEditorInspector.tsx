...
const collectIds = (node: ComponentNode, ids: Set<string> = new Set()): Set<string> => {
  ids.add(node.id)
  node.children?.forEach((child) => collectIds(child, ids))
  return ids
}

const renameNodeId = (node: ComponentNode, fromId: string, toId: string): ComponentNode => {
  if (node.id === fromId) {
    return { ...node, id: toId }
  }
  if (!node.children?.length) return node
  const nextChildren = node.children.map((child) => renameNodeId(child, fromId, toId))
...
export function BlueprintEditorInspector({ isCollapsed, onToggleCollapse }: BlueprintEditorInspectorProps) {
  ...
  const selectedId = useEditorStore((state) => state.blueprintStateByProject[blueprintKey]?.selectedId)
              ...
              <div className="InspectorField">
                <div className="InspectorFieldHeader">
                  <span className="InspectorLabel">
                    {t('inspector.fields.id.label', { defaultValue: 'Component ID' })}
                  </span>
                  <span className="InspectorDescription">
                    {t('inspector.fields.id.description', { defaultValue: 'Unique within the blueprint.' })}
                  </span>
                </div>
                  ...
                  {isDirty && (
                    <div className="InspectorFieldActions">
                      <button
                        ...
                        onClick={applyRename}
                        disabled={!canApply}
                        aria-label={t('inspector.actions.apply', { defaultValue: 'Apply' })}
                        title={t('inspector.actions.apply', { defaultValue: 'Apply' })}
                      >
                        <Check size={14} />
                      </button>
                    </div>
                ...
                {isDuplicate && (
                  <div className="InspectorWarning" role="alert">
                    ...
                    <span>{t('inspector.fields.id.duplicate', { defaultValue: 'ID already exists.' })}</span>
                  </div>
                )}
              </div>
            </div>
