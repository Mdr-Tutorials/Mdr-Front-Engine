...
export function ExportMirPage() {
    ...
    const projectType = useEditorStore(
        (state) =>
            (projectId ? state.projectsById[projectId]?.type : undefined) ??
            'project'
    );
    ...
    const renderTreeNodes = (nodes: FileTreeNode[], depth = 0): JSX.Element[] =>
        nodes.map((node) => {
            ...
            const isActive =
                Boolean(node.file) &&
                (activeReactFile || reactBundle?.entryFilePath) ===
                    node.file?.path;
            const fileIcon =
                    ...
                    >
                        {isFolder ? (
                            isExpanded ? (
                                <ChevronDown
                                    size={12}
                                    className="shrink-0 text-(--color-6)"
                                />
                            ) : (
                                <ChevronRight
                                    size={12}
                        ...
                        {isFolder ? (
                            isExpanded ? (
                                <FolderOpen
                                    size={13}
                                    className="shrink-0 text-(--color-7)"
                                />
                            ) : (
                                <Folder
                                    size={13}
                            ...
                            <span className="shrink-0 text-(--color-7)">
                                {fileIcon}
                            </span>
                        )}
