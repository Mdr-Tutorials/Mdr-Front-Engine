...
describe("ProjectHome", () => {
  beforeEach(() => {
    ...
    resetEditorStore({
      projectsById: {
        "project-1": { id: "project-1", name: "Project Alpha", description: "Demo" },
      },
    })
  })
  ...
  it("shows project identity and routes to blueprint", () => {
    ...
    const blueprintLabel = screen.getByText("projectHome.actions.blueprint.label")
    ...
    expect(navigateMock).toHaveBeenCalledWith("/editor/project/project-1/blueprint")
  })
  ...
  it("disables actions when no project id is available", () => {
    ...
    const settingsButton = screen.getByRole("button", { name: "projectHome.actions.settings.label" })
    const blueprintLabel = screen.getByText("projectHome.actions.blueprint.label")
    const blueprintButton = blueprintLabel.closest("button")
