  ...
  it('publishes a private project and keeps community entry in new tab', async () => {
    publishProjectMock.mockResolvedValue({
      project: {
        ...
        name: 'Project Alpha',
        description: 'Demo',
        mir: { version: '1.0', ui: { root: { id: 'root', type: 'container' } } },
        isPublic: true,
        starsCount: 3,
    ...
    fireEvent.click(screen.getByRole('button', { name: 'projectHome.visibility.publish' }));

    await waitFor(() => {
      expect(publishProjectMock).toHaveBeenCalledWith('token-1', 'project-1');
    });

    expect(useEditorStore.getState().projectsById['project-1']?.isPublic).toBe(true);
