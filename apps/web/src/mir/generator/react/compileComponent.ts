...
const buildBuiltInInlineHandler = (
    ...
    if (action === 'executeGraph')
        return buildExecuteGraphInlineHandler(params);
    return null;
};
...
export const compileMirToReactComponent = (
    ...
    const propFunctionKeys = Object.entries(propsDef)
        .filter(([, def]) => {
            ...
            return (
                type.includes('=>') || type.toLowerCase().includes('function')
            );
        })
        .map(([key]) => key);
    ...
    const compileNode = (node: ComponentNode, indent = '    '): string => {
        ...
        if (node.events) {
            Object.entries(node.events).forEach(([eventKey, eventDef]) => {
                ...
                if (
                    eventDef?.action &&
                    propFunctionKeys.includes(eventDef.action)
                ) {
                    propsArray.push(
                        `${reactEventName}={${toIdentifier(eventDef.action)}}`
                    );
                    return;
                }
    ...
    const interfaceFields = Object.entries(propsDef)
        .map(
            ([key, value]) => `  ${toIdentifier(key)}?: ${value.type || 'any'};`
        )
        .join('\n');
