...
export type ComponentAdapter = {
    ...
    applySelection?: (props: Record<string, any>, selectionData: Record<string, string>) => Record<string, any>;
};
...
export type ComponentRegistry = {
    register: (type: string, component: React.ElementType, adapter?: ComponentAdapter) => void;
    get: (type: string) => RegistryEntry | undefined;
    resolve: (type: string) => ResolvedComponent;
};

const normalizeSelectionData = (selectionData?: Record<string, string>) => selectionData ?? {};

const applyHtmlSelection = (props: Record<string, any>, selectionData: Record<string, string>) => {
    const { dataAttributes, ...rest } = props;
    const dataProps = typeof dataAttributes === 'object' && dataAttributes ? dataAttributes : {};
    return { ...rest, ...dataProps, ...normalizeSelectionData(selectionData) };
};

const applyMdrSelection = (props: Record<string, any>, selectionData: Record<string, string>) => {
    const dataProps = typeof props.dataAttributes === 'object' && props.dataAttributes ? props.dataAttributes : {};
    return {
        ...props,
        dataAttributes: { ...dataProps, ...normalizeSelectionData(selectionData) },
    };
};
...
export const htmlInputAdapter: ComponentAdapter = {
    ...
    mapProps: ({ resolvedProps, resolvedText }) => {
        ...
        if (resolvedText !== undefined && props.value === undefined && props.defaultValue === undefined) {
...
export const createComponentRegistry = (): ComponentRegistry => {
    ...
    const register = (type: string, component: React.ElementType, adapter: ComponentAdapter = htmlAdapter) => {
...
export const parseResolverOrder = (value?: string): RegistryGroup[] => {
    ...
    segments.forEach((segment) => {
        if ((segment === 'custom' || segment === 'mdr' || segment === 'native') && !order.includes(segment)) {
            order.push(segment);
        }
    });
