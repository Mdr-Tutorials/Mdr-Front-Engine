...
export function CommunityDetailPage() {
  ...
  const [project, setCommunityProject] = useState<CommunityProjectDetail | null>(
    null
  );
            ...
            <header className="rounded-3xl border border-black/10 bg-white/90 p-7 shadow-[0_10px_24px_rgba(0,0,0,0.06)] backdrop-blur">
              ...
              <div className="mt-5 flex flex-wrap items-center gap-3 text-xs text-black/60">
                <span className="rounded-full border border-black/15 bg-white px-3 py-1">
                  {t('detail.author', 'Author')}: {project.authorName || 'Unknown'}
                </span>
                <span className="rounded-full border border-black/15 bg-white px-3 py-1">
                  {t('detail.updated', 'Updated')}: {formatTime(project.updatedAt)}
                </span>
            ...
            <section className="grid gap-5 lg:grid-cols-[1.1fr_1fr]">
              <article className="rounded-3xl border border-black/10 bg-white p-4 shadow-[0_10px_20px_rgba(0,0,0,0.04)]">
                ...
                <div className="overflow-hidden rounded-2xl border border-black/10 bg-white p-3">
                  <MIRRenderer node={project.mir.ui.root} mirDoc={project.mir} />
                </div>
              </article>
