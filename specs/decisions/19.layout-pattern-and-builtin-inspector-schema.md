...
- [ ] 新增 `layoutPattern.types.ts`：定义 `LayoutPatternDefinition`、参数类型、update 协议。
- [ ] 新增 `builtInMeta.types.ts`：定义内置 Inspector Schema 类型。
- [ ] 明确 dataAttributes 约束（值统一字符串，避免类型分歧）。
- [ ] 补充 ADR 附录中的命名规范（pattern id、role、version）。
...
- [ ] 新增 `LayoutPatternRegistry`（register/get/list）。
- [ ] 实现首批范式（建议：`split`, `holy-grail`, `dashboard-shell`）。
- [ ] 在组件库左侧新增“布局范式”分组（与现有 layout 组并存）。
- [ ] `createNodeFromPaletteItem` 支持 pattern item，调用 `build(params)` 生成节点。
...
- [ ] 新增 `LayoutPatternPanel`，匹配 `data-layout-pattern-root`。
- [ ] 参数面板支持 `gap/padding/sidebarWidth/columns` 等通用参数。
- [ ] 参数更新调用 `LayoutPatternRegistry.update`，批量回写子树。
- [ ] 更新后保持当前选中节点稳定（必要时回退到 root 节点）。
