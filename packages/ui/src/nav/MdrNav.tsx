...
function MdrNav({
  ...
  const fullClassName =
    `MdrNav Columns-${columns} ${isFloat ? 'Float' : ''} ${canHide ? 'CanHide' : ''} ${backgroundStyle} ${className || ''}`.trim();
...
function MdrNavLeft({ children }: MdrNavAreaProps) {
  return <div className="MdrNavLeft">{children}</div>;
}

function MdrNavCenter({ children }: MdrNavAreaProps) {
  return <div className="MdrNavCenter">{children}</div>;
}

function MdrNavRight({ children }: MdrNavAreaProps) {
  return <div className="MdrNavRight">{children}</div>;
}

function MdrNavHeading({ heading }: { heading: string }) {
  return <h1 className="MdrNavHeading">{heading}</h1>;
}
