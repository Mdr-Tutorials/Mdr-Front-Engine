...
export interface MdrCollapseProps
    extends MdrComponent,
        MdrCollapseSpecificProps {}
        ...
        >
            {items.map((item) => {
                ...
                return (
                    <div
                        key={item.key}
                        className={`MdrCollapseItem ${isOpen ? 'Open' : ''} ${item.disabled ? 'Disabled' : ''}`}
                    >
                        <button
                            type="button"
                            className="MdrCollapseHeader"
                            onClick={() =>
                                !item.disabled && toggleKey(item.key)
                            }
                        >
                            <span>{item.title}</span>
                            <span className="MdrCollapseIcon">
                                {isOpen ? (
                                    <Minus size={14} />
                                ) : (
                                    <Plus size={14} />
                        ...
                        {isOpen && (
                            <div className="MdrCollapseContent">
                                {item.content}
                            </div>
                        )}
                    </div>
