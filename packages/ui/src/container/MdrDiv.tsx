...
function MdrDiv({
    ...
    const fullClassName = `MdrDiv ${display} ${overflow} ${textAlign ? textAlign : ''} ${className || ''}`.trim();

    const dataProps = { ...dataAttributes }

    const customStyle: React.CSSProperties = {
        ...(style as React.CSSProperties),
        ...(flexDirection && { flexDirection: flexDirection.toLowerCase() as React.CSSProperties['flexDirection'] }),
        ...(justifyContent && { justifyContent: justifyContent.toLowerCase().replace(/([A-Z])/g, '-$1').toLowerCase() as React.CSSProperties['justifyContent'] }),
        ...(alignItems && { alignItems: alignItems.toLowerCase() as React.CSSProperties['alignItems'] }),
