...
function MdrInput({
    ...
    const fullClassName = `MdrInput ${size} ${state ? state : ''} ${disabled ? 'Disabled' : ''} ${readOnly ? 'ReadOnly' : ''} ${icon ? 'WithIcon' : ''} ${iconPosition} ${className || ''}`.trim();
    ...
    if (icon) {
        return (
            <div className="MdrInput-wrapper">
                {iconPosition === 'Left' && <span className="MdrInput-icon">{icon}</span>}
                {inputElement}
                {iconPosition === 'Right' && <span className="MdrInput-icon">{icon}</span>}
            </div>
        );
    }
