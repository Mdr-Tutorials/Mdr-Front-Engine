...
interface MdrImageGallerySpecificProps {
    ...
    selectable?: boolean,
    maxSelection?: number,
    onImageClick?: (image: MdrImageGallerySpecificProps['images'][0], index: number) => void;
    onSelectionChange?: (selectedIndices: number[]) => void;
}

export interface MdrImageGalleryProps extends MdrComponent, MdrImageGallerySpecificProps { }
...
function MdrImageGallery({
    ...
    const fullClassName = `MdrImageGallery ${layout} ${gap} ${className || ''}`.trim();

    const dataProps = { ...dataAttributes }

    const handleImageClick = (image: MdrImageGallerySpecificProps['images'][0], index: number) => {
        if (onImageClick) {
