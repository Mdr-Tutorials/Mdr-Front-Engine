...
export interface MdrRegionPickerProps extends MdrComponent, MdrRegionPickerSpecificProps { }
...
function MdrRegionPicker({
    ...
    const [internalValue, setInternalValue] = useState<MdrRegionValue>(defaultValue || {});
    ...
    const selectedProvince = provinces.find((item) => item.value === currentValue.province);
    ...
    const handleDistrictChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        emitChange({ province: currentValue.province, city: currentValue.city, district });
    };

    const fullClassName = `MdrRegionPicker ${size} ${state} ${disabled ? 'Disabled' : ''} ${className || ''}`.trim();
