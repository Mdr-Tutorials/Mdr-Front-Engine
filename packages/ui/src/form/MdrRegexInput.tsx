...
export interface MdrRegexInputProps extends MdrComponent, MdrRegexInputSpecificProps { }
...
function MdrRegexInput({
    ...
    const fullClassName = `MdrRegexInput ${size} ${resolvedState} ${disabled ? 'Disabled' : ''} ${className || ''}`.trim();
    const dataProps = { ...dataAttributes };

    const helperMessage = message || (currentValue ? (isValid ? validMessage : invalidMessage) : undefined);
    ...
    return (
        <div className={`MdrField ${fullClassName}`} style={style as React.CSSProperties} id={id} {...dataProps}>
            ...
            />
            {helperMessage && <div className={`MdrFieldMessage ${resolvedState}`}>{helperMessage}</div>}
        </div>
    );
}
