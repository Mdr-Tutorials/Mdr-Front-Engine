...
export interface MdrFileUploadProps
  extends MdrComponent,
    MdrFileUploadSpecificProps {}
...
function MdrFileUpload({
  ...
  const fullClassName =
    `MdrFileUpload ${disabled ? 'Disabled' : ''} ${className || ''}`.trim();
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="MdrFileUploadText">Click or drag files to upload</div>
        ...
        <ul className="MdrFileUploadList">
          {files.map((file) => (
            <li key={`${file.name}-${file.size}`} className="MdrFileUploadItem">
              ...
              <span className="MdrFileUploadSize">
