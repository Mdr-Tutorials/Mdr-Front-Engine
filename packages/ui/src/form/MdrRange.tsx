...
function MdrRange({
    ...
    const [internalValue, setInternalValue] = useState<MdrRangeValue>(defaultValue || { min, max });
    ...
    const fullClassName = `MdrRange ${disabled ? 'Disabled' : ''} ${className || ''}`.trim();
        ...
        <div className={`MdrField ${fullClassName}`} style={style as React.CSSProperties} id={id} {...dataProps}>
            {label && (
                <div className="MdrFieldHeader">
                    ...
                    {showValue && (
                        <span className="MdrRangeValue">{currentValue.min} - {currentValue.max}</span>
                    )}
                </div>
            )}
