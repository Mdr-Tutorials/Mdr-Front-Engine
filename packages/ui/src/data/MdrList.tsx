...
function MdrList({
    ...
    const fullClassName = `MdrList ${size} ${bordered ? 'Bordered' : ''} ${split ? 'Split' : ''} ${className || ''}`.trim();
    const dataProps = { ...dataAttributes };

    return (
        <ul className={fullClassName} style={style as React.CSSProperties} id={id} {...dataProps}>
                    ...
                    {renderItem ? (
                        ...
                        <>
                            <div className="MdrListContent">
                                ...
                                {item.description && <div className="MdrListDescription">{item.description}</div>}
                            </div>
                            {item.extra && <div className="MdrListExtra">{item.extra}</div>}
                        </>
                    )}
                </li>
            ))}
