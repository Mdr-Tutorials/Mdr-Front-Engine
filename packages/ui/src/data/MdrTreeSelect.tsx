...
export interface MdrTreeSelectProps extends MdrComponent, MdrTreeSelectSpecificProps { }

const flattenOptions = (options: MdrTreeSelectOption[], depth = 0): Array<{ option: MdrTreeSelectOption; depth: number }> => {
...
function MdrTreeSelect({
    ...
    const handleChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        const selected = flatOptions.find((item) => item.option.id === nextValue)?.option;
        if (onChange) {
            onChange(nextValue, selected);
        }
    };

    const fullClassName = `MdrTreeSelect ${disabled ? 'Disabled' : ''} ${className || ''}`.trim();
