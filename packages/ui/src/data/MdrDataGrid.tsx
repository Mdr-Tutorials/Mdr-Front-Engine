...
export interface MdrDataGridProps<T = Record<string, unknown>> extends MdrComponent, MdrDataGridSpecificProps<T> { }
...
function MdrDataGrid<T extends Record<string, unknown>>({
    ...
    const fullClassName = `MdrDataGrid ${striped ? 'Striped' : ''} ${bordered ? 'Bordered' : ''} ${hoverable ? 'Hoverable' : ''} ${className || ''}`.trim();
    ...
    return (
        <div className={fullClassName} style={style as React.CSSProperties} id={id} {...dataProps}>
            {showHeader && (
                <div className="MdrDataGridHeader" style={{ gridTemplateColumns: columnTemplate }}>
                    {columns.map((column) => (
                        <div key={column.key} className={`MdrDataGridCell Align${column.align || 'Left'}`}>
                            {column.title}
                        </div>
                    ))}
                </div>
